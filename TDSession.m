//
//  TDSession.m
//  TDAudioStreamer
//
//  Created by Tony DiPasquale on 11/15/13.
//  Copyright (c) 2013 Tony DiPasquale. The MIT License (MIT).
//

@import MultipeerConnectivity;

#import "TDSession.h"

@interface TDSession () <MCSessionDelegate, MCBrowserViewControllerDelegate>

@property (strong, nonatomic) MCSession *session;
@property (strong, nonatomic) MCAdvertiserAssistant *advertiser;
@property (strong, nonatomic) MCPeerID *peerID;

@end

@implementation TDSession

- (instancetype)initWithPeerDisplayName:(NSString *)name
{
    self = [super init];
    if (!self) return nil;

    // I set the Peer Name for the multpeer session
    self.peerID = [[MCPeerID alloc] initWithDisplayName:name];

    return self;
}

#pragma mark - Properties

- (MCSession *)session
{
    NSLog(@" TDSession -> Session !");
    if (!_session) {
        _session = [[MCSession alloc] initWithPeer:self.peerID];
        _session.delegate = self;
    }
    return _session;
}

#pragma mark - MCSessionDelegate methods

- (void)session:(MCSession *)session peer:(MCPeerID *)peerID didChangeState:(MCSessionState)state
{
    if (state == MCSessionStateConnecting) {
        NSLog(@"Connecting to %@", peerID.displayName);
        [self.delegate didPeerChangeState:@"connecting"];
        
    } else if (state == MCSessionStateConnected) {
        NSLog(@"Connected to %@", peerID.displayName);
        [self.delegate didPeerChangeState:@"connected"];
        
    } else if (state == MCSessionStateNotConnected) {
        NSLog(@"Disconnected from %@", peerID.displayName);
        [self.delegate didPeerChangeState:@"disconnected"];
    }
}

/* Three kinds of data exchange */
- (void)session:(MCSession *)session didReceiveData:(NSData *)data fromPeer:(MCPeerID *)peerID
{
    // MESSAGES
    NSLog(@" TDSession -> I Received the Data");
    [self.delegate session:self didReceiveData:data];
}

- (void)session:(MCSession *)session didReceiveStream:(NSInputStream *)stream withName:(NSString *)streamName fromPeer:(MCPeerID *)peerID
{
    // STREAMING
}

- (void)session:(MCSession *)session didStartReceivingResourceWithName:(NSString *)resourceName fromPeer:(MCPeerID *)peerID withProgress:(NSProgress *)progress
{
    // RESOURCES
    NSLog(@"TDSession -> I Started Receiving Resource");
}

- (void)session:(MCSession *)session didFinishReceivingResourceWithName:(NSString *)resourceName fromPeer:(MCPeerID *)peerID atURL:(NSURL *)localURL withError:(NSError *)error
{
    // RESOURCES
    NSLog(@"TDSession -> I Finished Receiving Resource");
}

- (NSArray *)connectedPeers
{
    NSLog(@" TDSession -> Connected Peers: %@", [self.session connectedPeers]);
    return [self.session connectedPeers];
}

- (void)sendData:(NSData *)data
{
    NSLog(@" TDSession -> I'm sending data");
    NSError *error;
    [self.session sendData:data toPeers:self.session.connectedPeers withMode:MCSessionSendDataReliable error:&error];
    if (error) {
        NSLog(@"Error: %@", error.userInfo.description);
    }
}

#pragma mark - MCBrowserViewController delegate

- (void)browserViewControllerDidFinish:(MCBrowserViewController *)browserViewController
{
    NSLog(@" TDSession -> I finished the 'Near Device Selection'");
    [browserViewController dismissViewControllerAnimated:YES completion:nil];
}

- (void)browserViewControllerWasCancelled:(MCBrowserViewController *)browserViewController
{
    NSLog(@" TDSession -> I cancelled the 'Near Device Selection'");
    [browserViewController dismissViewControllerAnimated:YES completion:nil];
}

// Here I present the view of the near devices
- (MCBrowserViewController *)browserViewControllerForSeriviceType:(NSString *)type
{
    NSLog(@" TDSession -> Present the 'Near Device Selection'");
    MCBrowserViewController *view = [[MCBrowserViewController alloc] initWithServiceType:type session:self.session];
    view.delegate = self;
    return view;
}

#pragma mark - Advertising Assistant

- (void)startAdvertisingForServiceType:(NSString *)type discoveryInfo:(NSDictionary *)info
{
    NSLog(@" TDSession -> Start Advertising");
    
    if (!self.advertiser)
        self.advertiser = [[MCAdvertiserAssistant alloc] initWithServiceType:type discoveryInfo:info session:self.session];

    [self.advertiser start];
}

- (void)stopAdvertising
{
    NSLog(@" TDSession -> Stop Advertising");
    [self.advertiser stop];
}

@end
